...
const T& DenseTensor<T, P>::Get(const Index& index) const {
  const auto linear_index = internal::IndexHelper<P, P - 1>::LinearizeIndex(index, size_);
...
void DenseTensor<T, P>::Allocate() {
  const auto total_size =
      std::accumulate(size_.cbegin(), size_.cend(), 1, internal::MultiplyOrDie<std::uint64_t>);
